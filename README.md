...
2. [データプロトコル詳細](#データプロトコル詳細)
3. [座標系と角度変換](#座標系と角度変換)
4. [足検出アルゴリズム](#足検出アルゴリズム)
5. [音声合成システム](#音声合成システム)
6. [3D 鍵盤レンダリング](#3d-鍵盤レンダリング)
7. [WebSocket 通信と再接続](#websocket-通信と再接続)
8. [UI コントロール](#ui-コントロール)
9. [実行方法](#実行方法)
10. [トラブルシューティング](#トラブルシューティング)
11. [パラメータ調整ガイド](#パラメータ調整ガイド)
...
%%{init: {'theme':'base'}}%%
graph LR
    A["0<br/>(無音)"] -->|Attack<br/>10ms| B["0.3<br/>(最大)"]
    B -->|Sustain<br/>(継続)| C["0.3"]
    C -->|Decay<br/>(オプション)| D["減衰"]
    D -->|Release<br/>100ms| E["0<br/>(停止)"]
...
- **WebSocket**: 接続状態 (connecting/connected/disconnected/error)
- **Points**: 点数（常に360）
- **Update Rate**: フレームレート (Hz)
- **Frame Count**: 受信フレーム総数
- **Timestamp**: ESP32 のタイムスタンプ
- **Ping統計**: RTT (min/max/avg), カウント（直近30秒）

### 右上パネル: ピアノ設定

#### 可変パラメータ (スライダー)
- **内側半径** (0.1 - 3.0 m): ドーナツの内側
- **外側半径** (0.11 - 4.0 m): ドーナツの外側
- **鍵盤境界除外割合** (0% - 50%): 境界マージン

#### 音域シフト
- **−/+ボタン**: ±2オクターブの範囲で全体をシフト
- 範囲表示: `C3 〜 B4 (24音)` のように現在の音域を表示
...
```mermaid
graph TD
    Current[現在の実装] --> F1[録音・再生機能]
    Current --> F2[コード検出]
    Current --> F3[MIDI 出力]
    Current --> F4[エフェクト<br/>リバーブ/ディレイ]
    Current --> F5[楽譜表示]

    F2 -.-> F2A["複数鍵盤同時<br/>→ コード名表示"]
    F3 -.-> F3A["Web MIDI API<br/>→ 外部音源"]
    F5 -.-> F5A["五線譜に<br/>リアルタイム表示"]
...
- [ ] **録音・再生機能**: 演奏を記録して再生
- [ ] **コード検出**: 複数鍵盤を同時に踏んだときにコード名を表示
- [ ] **MIDI 出力**: Web MIDI API で外部音源に接続
- [ ] **エフェクト**: リバーブ、ディレイ、コーラス
- [ ] **楽譜表示**: 踏んだ鍵盤を五線譜にリアルタイム表示

### アルゴリズム改善

```mermaid
graph TD
    Current2[現在の検出] --> A1[機械学習<br/>足形状認識]
    Current2 --> A2[多人数対応]
    Current2 --> A3[ジェスチャー認識]
    Current2 --> A4[適応的閾値]

    A1 -.-> A1A["点群から<br/>足の形状を認識"]
    A2 -.-> A2A["複数の足を<br/>同時検出"]
    A3 -.-> A3A["足の動きで<br/>音量/ピッチ制御"]
    A4 -.-> A4A["環境に応じて<br/>自動調整"]

    style Current2 fill:#9f9
    style A1 fill:#f9f
    style A2 fill:#f9f
    style A3 fill:#f9f
    style A4 fill:#f9f
...
- [ ] **機械学習による足検出**: 点群から足の形状を認識
- [ ] **多人数対応**: 複数の足を同時検出
- [ ] **ジェスチャー認識**: 足の動きで音量やピッチベンドを制御
- [ ] **適応的閾値**: 環境に応じて自動調整
